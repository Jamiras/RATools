...
namespace RATools.Tests.Parser.Functions
{
    ...
    class RepeatedFunctionTests
    {
        ...
        {
            Evaluate("repeated(4, never(byte(0x1234) == 5) || once(byte(0x1234) == 34))", "ResetIf modifier not allowed in subclause");
            Evaluate("repeated(4, measured(repeated(6, byte(0x1234) == 5)) || byte(0x1234) == 34)", "Measured modifier not allowed in subclause");
        }
        ...
        [Test]
        public void TestNeverMany()
        {
            var requirements = Evaluate("repeated(10, byte(0x002345) == 1 && never(byte(0x001234) < 5) && never(byte(0x001234) == 7) && never(byte(0x001234) > 10))");
            Assert.That(requirements.Count, Is.EqualTo(4));
            Assert.That(requirements[0].Left.ToString(), Is.EqualTo("byte(0x001234)"));
            Assert.That(requirements[0].Operator, Is.EqualTo(RequirementOperator.LessThan));
            Assert.That(requirements[0].Right.ToString(), Is.EqualTo("5"));
            ...
            Assert.That(requirements[1].Left.ToString(), Is.EqualTo("byte(0x001234)"));
            Assert.That(requirements[1].Operator, Is.EqualTo(RequirementOperator.Equal));
            Assert.That(requirements[1].Right.ToString(), Is.EqualTo("7"));
            Assert.That(requirements[1].Type, Is.EqualTo(RequirementType.OrNext)); // optimizer was not called
            Assert.That(requirements[1].HitCount, Is.EqualTo(0));
            Assert.That(requirements[2].Left.ToString(), Is.EqualTo("byte(0x001234)"));
            Assert.That(requirements[2].Operator, Is.EqualTo(RequirementOperator.GreaterThan));
            Assert.That(requirements[2].Right.ToString(), Is.EqualTo("10"));
            Assert.That(requirements[2].Type, Is.EqualTo(RequirementType.ResetNextIf)); // optimizer was not called
            Assert.That(requirements[2].HitCount, Is.EqualTo(0));
